...
if st.session_state.rag_enabled:
    st.warning("RAG Context is Enabled (Feature to be implemented on backend side).")
else:
...
if "history_loaded" not in st.session_state or st.session_state.history_loaded is False:
    ...
    # Convert datetime strings back to datetime objects if needed, or format for display
...
if prompt := st.chat_input("Say something..."):
    ...
    with st.chat_message("assistant"):
        ...
        # full_response_content = "" # REMOVED: No longer needed here
        st.session_state.current_response = "" # Reset for new response

        async def get_streaming_response():
            # nonlocal full_response_content # REMOVED: No longer needed with st.session_state.current_response
            try:
                ...
                async for chunk in stream_generator:
                    st.session_state.current_response += chunk # MODIFIED: Directly append to session state
                ...
                # Save assistant message to database (after full response is received)
                st.session_state.messages.append({"role": "assistant", "content": st.session_state.current_response}) # Use final content
