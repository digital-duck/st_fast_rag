...
async def generate_stream(request: GenerateRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        await app.loop.run_in_executor(executor, create_chat_message, db, user_message_data)
        ...
        async def generate_response_chunks() -> AsyncGenerator[str, None]:
            ...
            # This needs to be done in a separate awaitable/task to not block streaming
            # or by running it in the executor.
            ...
            await app.loop.run_in_executor(executor, create_chat_message, db, assistant_message_data)
...
async def get_session_chat_history(session_id: str, db: Session = Depends(get_db)):
    ...
    history = await app.loop.run_in_executor(executor, get_chat_history, db, session_id)
    return history
...
async def create_new_note(note: NoteCreate, db: Session = Depends(get_db)):
    """Creates a new note."""
    db_note = await app.loop.run_in_executor(executor, create_note, db, note)
    return db_note

@app.get("/notes/", response_model=List[NoteResponse])
async def read_notes(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Retrieves all notes."""
    notes = await app.loop.run_in_executor(executor, get_notes, db, skip, limit)
    return notes

@app.get("/notes/{note_id}", response_model=NoteResponse)
async def read_note(note_id: int, db: Session = Depends(get_db)):
    """Retrieves a single note by ID."""
    db_note = await app.loop.run_in_executor(executor, get_note, db, note_id)
    if db_note is None:
...
async def update_existing_note(note_id: int, note: NoteUpdate, db: Session = Depends(get_db)):
    ...
    db_note = await app.loop.run_in_executor(executor, update_note, db, note_id, note)
    if db_note is None:
        raise HTTPException(status_code=404, detail="Note not found")
    return db_note
...
async def delete_existing_note(note_id: int, db: Session = Depends(get_db)):
    ...
    db_note = await app.loop.run_in_executor(executor, delete_note, db, note_id)
    if db_note is None:
        raise HTTPException(status_code=404, detail="Note not found")
    return db_note
